...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterions, optimizer):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        # Calculate accuracy
        total_correct_VR += ((pred_VR.argmax(dim=1) == true_VR) * mask_V).sum().item()
        total_correct_RR += ((pred_RR.argmax(dim=1) == true_RR) * mask_R).sum().item()
        total_correct_VT += ((pred_VT.argmax(dim=1) == true_VT) * mask_V).sum().item()
        total_correct_RT += ((pred_RT.argmax(dim=1) == true_RT) * mask_R).sum().item()

        total_masked_V += mask_V.sum().item()
        total_masked_R += mask_R.sum().item()
        ...
        if batch_id % args['print_every'] == 0:
            print(
                ...
                (epoch + 1, args['num_epochs'], batch_id + 1, len(data_loader), R_loss.item(), T_loss.item()),
                end='', flush=True
            )

    train_reactive_accuracy = total_correct_VR / total_masked_V
    train_template_accuracy = total_correct_VT / total_masked_V

    print(
        '\nepoch %d/%d, train reactive loss %.4f, template loss %.4f, reactive accuracy %.4f, template accuracy %.4f' %
        (epoch + 1, args['num_epochs'], train_R_loss / batch_id, train_T_loss / batch_id,
         train_reactive_accuracy, train_template_accuracy)
    )
...
def run_an_eval_epoch(args, model, data_loader, loss_criterions):
    ...
    val_reactive_accuracy = total_correct_VR / total_masked_V
    val_template_accuracy = total_correct_VT / total_masked_V

    print(
        'Validation loss %.4f, reactive accuracy %.4f, template accuracy %.4f' %
        (val_loss / batch_id, val_reactive_accuracy, val_template_accuracy)
    )
...
def main(args):
    if args['model_name'] == 'default':
        if args['sep']:
            args['model_name'] = 'DeepMech_sep.pth'
        else:
            args['model_name'] = 'DeepMech_mix.pth'
    else:
    ...
    #print('loss_criterions:', loss_criterions, '\n')
...
if __name__ == '__main__':
    ...
    pprint(args)  # Use pprint for clean output
    print('--------------------------')
