...
from dataset import USPTODataset, USPTOTestDataset, combine_reactants, USPTO_OOD_Dataset
...
def load_dataloader(args):
    if args['mode'] == 'train':
        ...
        test_set = USPTO_OOD_Dataset(args,
                            mol_to_graph=partial(mol_to_bigraph, add_self_loop=True),
                            node_featurizer=args['node_featurizer'],
                            edge_featurizer=args['edge_featurizer'])
        test_loader = DataLoader(dataset=test_set, batch_size=args['batch_size'],
...
def load_model(args):
    ...
    print ('Parameters of loaded DeepMech:')
    print (exp_config)
...
def collate_molgraphs(data):
    ...
    #print('bond_dicts:', bonds_dicts['virtual'][0].shape, bonds_dicts['real'][0].shape)
