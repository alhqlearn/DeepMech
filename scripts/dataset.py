...
class USPTO_OOD_Dataset(object):
    def __init__(self, args, mol_to_graph, node_featurizer, edge_featurizer, load=True, log_every=1000):
        df = pd.read_csv('%s/preprocessed_oob.csv' % args['data_dir'])
        self.reactants = df['Reactants'].tolist()
        self.reagents = df['Reagents'].tolist()
        self.sep = args['sep']
        self.fgraph_path = '../data/saved_graphs/oob_%s_fgraph.bin' % args['dataset']
        if self.sep:
            self.dgraph_path = '../data/saved_graphs/oob_%s_dgraph_sep.pkl' % args['dataset']  # changed by numpy matrix for faster processing speed
        else:
            self.dgraph_path = '../data/saved_graphs/oob_%s_dgraph_mix.pkl' % args['dataset']
        self._pre_process(mol_to_graph, node_featurizer, edge_featurizer)
