...
def label_foward_edit_site(reactants, products, edit_num):
    #print('reactants, products, edit_num:\n', reactants, products, edit_num)
    '''
    example input: reactants = atom_mapped_reactant_smiles
    products = atom_mapped_product_smiles
    edit_num = ['10', '15', ..]
    '''
    edit_num = [int(num) for num in edit_num]
    rmol = Chem.MolFromSmiles(reactants)
    ...
    for a in edit_num:
        for b in edit_num:
            ...
            try:
                ...
                pbond = None
            #print('mapp:', a, b)
            #print('checking here:', ratom_map[a], ratom_map[b])
            rbond = rmol.GetBondBetweenAtoms(ratom_map[a], ratom_map[b])
            #print('rbond:', rbond)
            if check_bond_break(rbond, pbond):
                ...
                if b in patom_map:
                    ...
                    acceptors1.add(b)
                break_bond = True
    #print('broken_bonds.append\n', broken_bonds)
    #print('acceptors1:--->', acceptors1) 
    # change bond
    for a in edit_num:
        for b in edit_num:
            if a >= b:
    ...
    #print('used_atoms:', used_atoms)
    remote_atoms = [atom for atom in edit_num if atom not in used_atoms]
    remote_bonds = []
    #print('remote_atoms:', remote_atoms)
    for a in remote_atoms:
        atom = rmol.GetAtomWithIdx(ratom_map[a])
        neighbors_map = [n.GetAtomMapNum() for n in atom.GetNeighbors()]
        #print('neighbors_map:', neighbors_map)
        connected_neighbors = [b for b in used_atoms if b in neighbors_map]
        #print('number of connected_neighbors:', len(connected_neighbors))
        if len(connected_neighbors) > 0:
            #print('we are here')
            pass
        else: #this else condition is written by us
            if not neighbors_map: #check if there are no neighbors
                if len(remote_atoms) > 1:
                    the_other_atom = next(x for x in remote_atoms if x != a) #if there is a single neighbor, its a problem
                    remote_bonds.append((a, the_other_atom)) #remember this will be a virtual bond
                else:
                    the_other_atom = next(x for x in used_atoms if x != a)
                    #print('the other atom here:', (a, the_other_atom))
                    remote_bonds.append((a, the_other_atom)) #remember this will be a virtual bond
            else:
                for n in neighbors_map:
...
def label_CHS_change(smiles1, smiles2, edit_num, replacement_dict, use_stereo):
    ...
    for atom_map in edit_num:
        ...
        if atom_map in atom_map_dict2:
            ...
            #print('atom1.GetNumExplicitHs():', atom1.GetNumExplicitHs())
            #print('Check the atom:', atom1.GetAtomicNum(), atom1.GetFormalCharge())
            ...
            #print('atom2.GetNumExplicitHs():', atom2.GetNumExplicitHs())
            #print('Check the atom:', atom2.GetAtomicNum(), atom2.GetFormalCharge())
    #print('H_dict:', H_dict)
...
def bondmap2idx(bond_maps, idx_dict, temp_dict, sort = False, remote = False):
    #print('Input to the bondmap2idx:\n', bond_maps, idx_dict, temp_dict) 
...
def match_label(reactants, products, replacement_dict, edit_num, retro = True, remote = True, use_stereo = True):
    ...
    #print('input to the label_CHS_change function:', smiles1, smiles2, edit_num, replacement_dict)
    ...
    if retro:
        ...
        #print('formed_bonds, broken_bonds, changed_bonds, remote_bonds:\n', formed_bonds, broken_bonds, changed_bonds, remote_bonds)
        edits = {'A': bondmap2idx(formed_bonds, atom_map_dict, replacement_dict),
                  'B': bondmap2idx(broken_bonds, atom_map_dict, replacement_dict),
                  'C': bondmap2idx(changed_bonds, atom_map_dict, replacement_dict)}
        if remote:
            #print('remote_bonds, atom_map_dict, replacement_dict:--->', remote_bonds, atom_map_dict, replacement_dict, '\n')
