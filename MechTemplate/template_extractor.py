...
#from template_extract_utils import atom_neighbors, match_label

default_setting = {'verbose': False, 'use_stereo': False, 'use_symbol': False, 'max_unmap': 5, 'retro': False, 'remote': True, 'least_atom_num': 0}
...
def ring_smarts_inversion(labels, bonds, template):
  ring_opening_closing = re.findall(r'(\[A:\d+\])(1|[:#-]1)', template)
  labels_to_add_1 = [i[0] for i in ring_opening_closing]
  labels = [label + '1' if label in labels_to_add_1 else label for label in labels]
  # Initialize the output list
  output = []

  # Flag to track if we've found the first ]x match
  found_first = False

  # Iterate through labels and bonds2 simultaneously
  for i, (label, bond) in enumerate(zip(labels, bonds)):
      # Check if this label matches the ']x' pattern
      match = re.search(r'\](\d)', label)

      if match:
          if not found_first:
              #print('now here')
              # First ]x match, insert bond between ] and x
              output.append(label[:-1] + bond + label[-1])
              found_first = True
          else:
              #print('herex')
              # After the first match, insert bond between ] and x
              #output.append(label[:-1] + bond + label[-1])
              output.append(bond + label)
      else:
          if found_first:
              #print('here')
              # Prepend the bond before the label
              output.append(bond + label)
          else:
              #print('here2')
              # Postpend the bond after the label
              output.append(label + bond)  
...
def inverse_template(template):
    ...
    if ']1' in template:
        ...
        ring_template = False
    #bonds1 = [sorted(bond)[1] for bond in re.findall(r']([-=#:])|]1([-=#:])', template)]
    template_no_labels = re.sub(r'\[.*?\]', '', template) #I added these two line to get the bonds
    bonds1 = re.findall(r'[-#=:]', template_no_labels)
    bonds2 = bonds1[::-1]
    #print('score_bonds(bonds1) <= score_bonds(bonds2)', score_bonds(bonds1) <= score_bonds(bonds2))
    if len(bonds1) == 0 or ')' in template or score_bonds(bonds1) <= score_bonds(bonds2):
        return template
    ...
    if ring_template:
      #print('labels and bonds:', labels, '\n', bonds2, '\n', template)
      inv_template = ring_smarts_inversion(labels, bonds2, template)
      #print('inverted template:', inv_template)
      return inv_template
    else:
      for i in range(len(bonds2)):
        inv_template += bonds2[i]
...
#         regex = re.compile("|".join(map(re.escape, replacement_dict_symbol.keys())))
#         transform_newmaps = regex.sub(lambda match: replacement_dict_symbol[match.group(0)], transform)
...
def extend_atom_tag(reactant, max_num):
    ...
    '''
    This we didn't want, so stopped it, why?
...
def count_brackets(entry):
    # Split the entry at ">>"
    before_after = entry.split('>>')
    
    # Define a regex pattern to find elements in square brackets
    pattern = r'\[.*?\]'
    
    # Find all elements within square brackets before and after the ">>" sign
    before = re.findall(pattern, before_after[0])
    after = re.findall(pattern, before_after[1])
...
def extract_from_reaction(reaction, setting = default_setting):
    ...
    #print('changed_atoms, changed_atom_tags, err', changed_atoms, changed_atom_tags, err)        
    if err: 
        if VERBOSE:
    ...
    if not changed_atom_tags:
        ...
        result = {'products': products_smiles, 'reactants': reactants_smiles,
                'necessary_reagent': clean_map_and_sort(reagents_list),
                'reaction_smarts': '[A:1]>>[A:1]',
                 'intra_only': False,
                 'dimer_only': False,
                 ...
                 'Charge_change': {1:0},
                 'Chiral_change': {1:0}}
        return result #{'reaction_id': reaction['_id']}
    ...
    for atom in changed_atoms:
        if atom.GetAtomicNum() == 1 and atom.GetFormalCharge() == -1:
            H_num = atom.GetAtomMapNum()
            rxn_string = rxn_string.replace("[#1]", f"[#1:{H_num}]")

    #print('atom_dict', atom_dict, 'rxn_string:', rxn_string)
    #rxn_string = '([A:10]).([#1:301])>>([A:10]).([#1:301])' 
    rxn_canonical, replacement_dict = canonicalize_transform(rxn_string, atom_dict)
    #print('rxn_canonical:', rxn_canonical)
...
#         pass
    ...
    x, y = count_brackets(canonical_template)
    if x != y:
        print('\nrxn id:\n', reaction['_id'], '\nrxn_string\n', rxn_string, '\nrxn_canonical', rxn_canonical)
        #print('\natom_dict:\n', atom_dict)
