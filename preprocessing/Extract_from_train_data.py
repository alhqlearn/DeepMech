...
def extract_templates(args, extractor):
    ...
    #norxn_templates = defaultdict(int)
        ...
        try:
            ...
            for edit_type in edits:
                ...
                if len(bonds) > 0:
                    ...
                    #for the 'R' I am adding in the virtual templates as well, this will cause duplicacy
                    if edit_type == 'R':
                        virtual_templates['%s_%s' % (template, edit_type)] += 1


            #all_bonds_empty = True

            #for edit_type in edits:
            #    bonds = edits[edit_type][0]
            #    if len(bonds) > 0:
            #        all_bonds_empty = False
            #        if edit_type != 'A':
            #            real_templates['%s_%s' % (template, edit_type)] += 1
            #        else:
            #            virtual_templates['%s_%s' % (template, edit_type)] += 1
            
            # After iterating through all edit_types, check the flag
            #if all_bonds_empty:
            #    print('NO reaction? check it:', reaction)
            #     norxn_templates['%s_%s' % (template, 'N')] += 1
            #    virtual_templates['%s_%s' % (template, 'N')] += 1 
            #     real_templates['%s_%s' % (template, 'N')] += 1
    ...
    #TemplateInfos.to_csv('../data/USPTO_480k/template_infos_demo.csv')
...
def export_template(derived_templates, args):
    for k in derived_templates.keys():
        ...
        #template_df.to_csv('../data/USPTO_480k/checking_templates_demo.csv')
