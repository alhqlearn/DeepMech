...
sys.path.append('../')
from MechTemplate.template_extractor import extract_from_reaction
...
def labeling_dataset(args, split, template_dicts, template_infos, extractor):
    ...
    for i, reaction in rxns.items():
        #print(f'look at here:{i}\n {reaction}')
        #print(len(reactants))
        reactant = reaction.split('>>')[0]
        reagent = ''
        rxn_labels_s = []
        rxn_labels_m = []
        #print(len(reactants), len(rxn_labels_s))
        try:
            ...
            if template not in template_infos.keys():
                ...
                continue
            #print('len reactants;', len(reactants), len(labels_sep))     
        ...
        if edit_n <= args['max_edit_n']:
            ...
            try:
                ...
                for edit_type in edit_bonds:
                    ...
                    for bond in bonds:
                        if edit_type != 'A':
                            ...
                            #this is just for the case of H+ transfer reactions; so I am allowing virtual bonds in the R action; not sure of the consequences
                            else:
                                if bond in real_sites_m:
                                    rxn_labels_s.append(('r', real_sites_s.index(bond), template_dicts['real']['%s_%s' % (template, edit_type)]))
                                    rxn_labels_m.append(('r', real_sites_m.index(bond), template_dicts['real']['%s_%s' % (template, edit_type)]))
                                elif bond in virtual_sites_m:
                ...
                #print(len(reactants), len(labels_mix))                
        
   
            except Exception as e:
                print ('look at here, some problem:', i, e) #, reaction)
                labels_sep.append(rxn_labels_s)
                labels_mix.append(rxn_labels_m)
                frequency.append(0)
                continue
        ...
        #print(reactants, labels_mix)

        #if len(reactants) != len(labels_mix):
        #    print(f"Length mismatch at item: {i}")
        #    print(f"reactants length: {len(reactants)}, labels_sep length: {len(labels_sep)}")
        #    break  # Exit the loop to inspect further
    ...
    print('lengths:', len(reactants), len(reagents), len(products), len(labels_sep), len(labels_mix), len(frequency))
    #print('the labels separators:', labels_sep)
    #print('infos:\n', reactants, '\n', labels_mix)
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser('Mechanistic Tempalte Preprocessing')
    ...
    #oob_pre = labeling_dataset(args, 'oob', template_dicts, template_infos, extractor)
